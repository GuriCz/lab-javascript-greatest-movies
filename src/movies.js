...
function getAllDirectors(movies) {
  ...
  function removeDuplicates(allDirectors) {
    return arr.filter((item, 
        index) => allDirectors.indexOf(item) === index);
}
  
console.log(removeDuplicates(allDirectors));
  
console.log(allDirectors);

// Iteration 2: Steven Spielberg. The best? - How many drama movies did STEVEN SPIELBERG direct?
const howManyMovies = movies.filter(movies => movies.director === 'Steven Spielberg' && movies.genre.includes("Drama"))

console.log(howManyMovies.length)

// Iteration 3: All scores average - Get the average of all scores with 2 decimals
function scoresAverage(movies) {
    const totalRatings = movies.reduce((acc, curr) => {
...
function dramaMoviesScore(moviesArray) {
    const allDramas = movies.filter(movies => movies.genre.includes("Drama"))
    const totalRatings = allDramas.reduce((acc, curr) => {
        return acc + curr.score;
    }, 0);
